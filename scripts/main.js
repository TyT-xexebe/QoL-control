							...
							table.table(cons((row) => {
								...
								row
									.button('?', Styles.cleart, () => {
										let desc = moduleDescriptions[mName] || 'No description available for ' + mName;
										let d = new BaseDialog(displayName);
										let t = new Table();
										t.add(desc).width(450).wrap().left();
										let p = new ScrollPane(t);
										d.cont.add(p).width(1200).height(600).row();
										d.addCloseButton();
										d.show();
									})
									.size(40, 40)
