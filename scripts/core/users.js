...
var _buf = null;
function getBuf() {
    if (!_buf) try { _buf = java.nio.ByteBuffer.allocate(4); } catch(e) {}
    return _buf;
}
function floatBits(f) {
    try {
        var b = getBuf(); if (!b) return 0;
        b.clear(); b.putFloat(new java.lang.Float(f));
        var r = b.getInt(0); return r < 0 ? r + 4294967296 : r;
    } catch(e) { return 0; }
}
function bitsFloat(bits) {
    try {
        var b = getBuf(); if (!b) return 0;
        b.clear(); b.putInt(new java.lang.Integer(bits > 2147483647 ? bits - 4294967296 : bits));
        return b.getFloat(0);
    } catch(e) { return 0; }
}
...
function extract(f) { return floatBits(f) & 0xFFF; }
function near(hx, target) { return Math.abs(hx - target) <= TOL; }
...
Events.run(Trigger.update, function() {
    ...
    Groups.player.each(function(p) {
        try {
            ...
            var isMatch = near(hx, BEACON) || near(hx, HELLO) || near(hx, HELLO_R);
            ...
            if (isMatch) {
                ...
                if (moved) {
                    trk.score = Math.max(trk.score - 2, 0);
                } else {
                    trk.score = Math.max(trk.score - 1, 0);
                }
            }

            if (trk.hasMoved && trk.score >= 25 && !trk.confirmed) {
                trk.confirmed = true;
