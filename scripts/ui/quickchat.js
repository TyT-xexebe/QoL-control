		...
		Vars.content.getBy(type).each((c) => {
			if (
				...
				allIcons.push({
					...
					order:
						typeOrder ||
						(c.category ? c.category.ordinal() * 100 : 0),
				});
			}
		});
	};
	...
	if (Core.settings.getBool('qol-quickchat-running', false)) {
		if (!currentSessionExecuting) {
			...
			if (defCmd.enabled) {
				...
				Timer.schedule(() => {
					Vars.ui.showInfo(
						'[scarlet]Auto-Execute commands were disabled because the game crashed or closed unexpectedly during their last execution.'
					);
				}, 2);
			}
			return;
...
function buildHUD() {
	...
	let btn = table
		.button(Icon.chat, Styles.clearNonei, () => {
			if (!isDragging) {
				if (!dialog) buildUI();
				else showMainMenu();
				dialog.show();
			...
			touchDragged(event, x, y, pointer) {
				...
				if (isDragging) {
					btnX = Mathf.clamp(
						event.stageX,
						25,
						Core.scene.getWidth() - 25
					);
					btnY = Mathf.clamp(
						event.stageY,
						25,
						Core.scene.getHeight() - 25
					);
					table.setPosition(btnX, btnY);
				}
				return true;
...
function showMainMenu() {
	...
	if (data.length === 0) {
		...
		data.forEach((cmd, index) => {
			...
			btnCell
				.size(cmd.isDefault ? 255 : 300, 60)
				.left()
				.padRight(10)
				.get()
				.setStyle(Styles.cleart);

			if (cmd.isDefault) {
				rowTable
					.button(
						cmd.enabled ? Icon.ok : Icon.cancel,
						Styles.cleari,
						() => {
...
function showEditDialog(index, cmdData) {
	...
	t.area(text, (txt) => (text = txt))
		.size(350, 200)
		.padTop(5)
		.colspan(cmdData.isDefault ? 1 : 2);
	t.row();
