...
Events.run(Trigger.draw, () => {
	...
	if (
		selBlock &&
		selBlock.name &&
		(selBlock.name.indexOf('overdrive') > -1 ||
			selBlock.name.indexOf('mass-driver') > -1)
	) {
			...
			Groups.build.each((bld) => {
				if (bld.team === pTeam) {
					...
					if (type === selBlock) {
						let r = type.range;
						if (
							type.phaseRangeBoost !== undefined &&
							bld.phaseHeat !== undefined
						) {
						...
						if (r && r > 0) {
							...
							if (type.name.indexOf('overdrive') > -1)
		...
		for (let i = 0; i < plans.length; i++) {
			...
			if (!plan.breaking && plan.block) {
				...
				if (r && r > 0) {
					...
					let color = Pal.accent;
					if (b.name) {
						if (b.name.indexOf('overdrive') > -1)
							color = Color.valueOf('feb380');
						else if (b.name.indexOf('force') > -1)
							color = Pal.shield;
						else if (b.name.indexOf('mend') > -1) color = Pal.heal;
						else if (b.category === Category.turret)
