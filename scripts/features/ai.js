...
const refreshTarget = (u) => {
	...
	let items = [],
		contentItems = Vars.content.items();
	...
	for (let i = 0; i < items.length; i++) {
		let e = items[i];
		let tile = e.wall
			? Vars.indexer.findClosestWallOre(u, e.it)
			: findClosestFreeOre(u, e.it);
		if (tile) {
			targetItem = e.it;
			targetTile = tile;
			return;
...
Events.run(Trigger.update, () => {
	...
	if (lockEnabled) {
		u.mineTile = lockTile;
		if (u.dst2(lockPos.x, lockPos.y) > 4)
			u.vel.trns(u.angleTo(lockPos.x, lockPos.y), u.type.speed);
		else u.vel.set(0, 0);
		u.lookAt(
			lockTile ? lockTile.worldx() : lockPos.x,
			lockTile ? lockTile.worldy() : lockPos.y
		);
		return;
	}
	...
	if (buildEnabled) {
		...
		if (
			...
			(!followTarget ||
				!followTarget.unit() ||
				followTarget.unit().plans.isEmpty())
		) {
			let closest = null,
				minD = Infinity;
		...
		if (followTarget && followTarget.unit()) {
			...
			if (u.dst2(l) > 2500) u.vel.trns(u.angleTo(l), u.type.speed);
			else u.vel.set(0, 0);
			u.lookAt(l);
			return;
		}
	...
	if (mineEnabled && u.canMine()) {
		...
		if (
			isFull ||
			(targetItem && u.stack.amount > 0 && u.stack.item !== targetItem)
		) {
			u.mineTile = null;
			...
			let tx = targetTile.worldx(),
				ty = targetTile.worldy();
			if (!u.within(tx, ty, u.type.mineRange * 0.8))
				u.vel.trns(u.angleTo(tx, ty), u.type.speed);
			else u.vel.set(0, 0);
			u.lookAt(tx, ty);
		}
	}
...
interceptor.add('ai', (a) => {
	...
	switch (a[1]) {
				...
				if (f && f.team() == Vars.player.team()) {
					...
					notify(
						'[scarlet]player [white]' +
							a[2] +
							' [scarlet]not found or not in your team'
					);
			} else {
			...
			if (
				...
				for (let i = 2; i < a.length; i++) {
					if (allowedItems.hasOwnProperty(a[i])) {
						...
						changed.push(
							(allowedItems[a[i]] ? '[green]' : '[scarlet]') +
								a[i]
						);
					}
				}
		...
		case 's':
			...
			let isErekir =
				u == null ? false : u.type.name.match(/evoke|incite|emanate/);
			for (let i = 0; i < items.size; i++) {
				let it = items.get(i);
				let mineTier = u == null ? 0 : u.type.mineTier;
				if (
					!allowedItems.hasOwnProperty(it.name) ||
					it.hardness > mineTier
				)
					continue;
			...
			let bTrg = followTarget
				? (manualTarget ? '[accent]' : '[green]') +
