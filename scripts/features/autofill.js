				...
				if (ammoTypes) {
					...
					for (let i = 0; i < allItems.size; i++) {
						...
						if (ammoTypes.containsKey(item)) {
							...
							let pIndex = GLOBAL_AMMO_PRIORITY.indexOf(
								item.name
							);
							let score =
								pIndex !== -1
									? 1000 - pIndex
									: (bullet.damage + bullet.splashDamage) *
										(bullet.ammoMultiplier || 1) *
										(bullet.reloadMultiplier || 1);
...
const autofillHandler = (args) => {
	...
	notify(
		'[lightgray]Autofill ' +
			(autofillEnabled ? '[green]ON' : '[scarlet]OFF')
	);
};
	...
	if (domeBlock) {
		Groups.build.each((b) => {
			if (
				b.block === domeBlock &&
				b.team === p.team() &&
				b.within(px, py, buildRange)
			) {
				if (
					b.items &&
					(b.items.get(siliconItem) === 0 ||
						b.items.get(phaseItem) === 0)
				) {
					targets.push(b);
				}
			}
	...
	if (nearCore && !hasItem) {
		for (let i = 0; i < targets.length; i++) {
			...
			for (let j = 0; j < bestItems.length; j++) {
				...
				if (t.block === domeBlock && t.items && t.items.get(item) > 0)
					continue;
