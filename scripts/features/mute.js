...
function loadMuted() {
	try {
		...
		return parsed.map((item) =>
			typeof item === 'string' ? { type: 'exact', value: item } : item
		);
	} catch (e) {
		return [];
	}
...
function isMuted(name) {
	...
	return muted.some((m) =>
		m.type === 'exact' ? cleanName === m.value : cleanName.includes(m.value)
	);
}
		...
		if (e.player && isMuted(e.player.name)) {
			Core.app.post(() => {
				try {
					...
					let messagesField = Vars.ui.chatfrag
						.getClass()
					...
					for (let i = 0; i < messages.size; i++) {
						...
						if (typeof msgObj.getClass === 'undefined') {
							if (
								Strings.stripColors(String(msgObj)).indexOf(
									cleanPlayerName
								) !== -1
							)
								isMatch = true;
							...
							try {
								...
								if (sender) {
									...
									if (
										senderName &&
										Strings.stripColors(senderName) ===
											cleanPlayerName
									)
										isMatch = true;
								} else {
									let textField = msgObj
										.getClass()
										.getDeclaredField('message');
									textField.setAccessible(true);
									if (
										Strings.stripColors(
											textField.get(msgObj)
										).indexOf(cleanPlayerName) !== -1
									)
										isMatch = true;
								}
							} catch (ex) {
					...
					if (targetIndex !== -1) {
						...
						try {
							...
							try {
								let updateChatMethod = Vars.ui.chatfrag
									.getClass()
									.getDeclaredMethod('updateChat');
...
interceptor.add('mute', (args) => {
	...
	if (cmd === 'list') {
		...
		if (found) {
			...
			if (
				!muted.some((m) => m.type === 'exact' && m.value === cleanName)
			) {
