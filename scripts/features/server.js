	...
	try {
		if (Vars.net.active() && Vars.net.client()) {
			...
			if (!ip || ip === 'undefined' || ip === '') {
				...
				port = parseInt(
					Core.settings.getString('qol-last-port', '6567')
				);
			}
...
function showMainMenu() {
	...
	if (keys.length === 0) {
		...
		keys.forEach((cat) => {
			...
			rowTable
				.button('[accent]' + cat, Styles.cleart, () =>
					showCategory(cat)
				)
				.size(300, 50)
				.left()
...
function showCategory(catName) {
	...
	header
		.button(Icon.left, Styles.cleari, () => showMainMenu())
		.size(50, 50)
		.padRight(10);
	header
	...
	if (servers.length === 0) {
		...
		servers.forEach((srv, srvIndex) => {
			...
			let isCurrent =
				current &&
				current.ip === normSrv.ip &&
				current.port === normSrv.port;
...
function showEditServerDialog(catName, index, srvData) {
	...
	if (index === -1 && current) {
		d.cont
			.button('Fill Current Server', Icon.download, () => {
				try {
...
function connectToServer(ip, port) {
	...
	Time.runTask(
		15,
		run(() => Vars.ui.join.connect(norm.ip, norm.port))
	);
}
