...
function showMapDialog() {
	...
	activeTeams.each(
		cons((teamData) => {
			...
			if (tr) {
				...
				if (tr.buildSpeedMultiplier !== 1)
					diffs.push('BuildSpd: x' + tr.buildSpeedMultiplier);
				if (tr.blockHealthMultiplier !== 1)
					diffs.push('BlkHP: x' + tr.blockHealthMultiplier);
				if (tr.blockDamageMultiplier !== 1)
					diffs.push('BlkDmg: x' + tr.blockDamageMultiplier);
				if (tr.unitBuildSpeedMultiplier !== 1)
					diffs.push('UnitBuildSpd: x' + tr.unitBuildSpeedMultiplier);
				if (tr.unitHealthMultiplier !== 1)
					diffs.push('UnitHP: x' + tr.unitHealthMultiplier);
				if (tr.unitDamageMultiplier !== 1)
					diffs.push('UnitDmg: x' + tr.unitDamageMultiplier);
	...
	if (customTeamRules.length > 0) {
		...
		for (let i = 0; i < customTeamRules.length; i++) {
			addRow(
				t,
				customTeamRules[i].name,
				customTeamRules[i].text,
				dynamicWrapWidth
			);
		}
	}
	...
	addRow(
		t,
		'Enemy Core Build Radius',
		rules.enemyCoreBuildRadius / 8 + ' tiles'
	);
	...
	let bannedB = [];
	rules.bannedBlocks.each(
		cons((b) => bannedB.push(b.emoji() + ' ' + b.localizedName))
	);
	addRow(
		t,
		'Banned Blocks',
		bannedB.length > 0 ? bannedB.join(', ') : 'None',
		dynamicWrapWidth
	);

	let bannedU = [];
	rules.bannedUnits.each(
		cons((u) => bannedU.push(u.emoji() + ' ' + u.localizedName))
	);
	addRow(
		t,
		'Banned Units',
		bannedU.length > 0 ? bannedU.join(', ') : 'None',
		dynamicWrapWidth
	);
	...
	dialog.cont
		...
		.height(Core.scene.getHeight() * 0.9);
	dialog.show();
}
