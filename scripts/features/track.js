	...
	if (trackRtsEnabled) {
		...
		if (unitScanTimer > 15) {
			...
			Groups.unit.each((u) => {
				...
				if (
					!p &&
					ctrl instanceof Packages.mindustry.ai.types.FormationAI
				) {
				...
				if (
					!p &&
					hasCommandAI &&
					ctrl instanceof Packages.mindustry.ai.types.CommandAI
				) {
				...
				if (p) {
					...
					try {
						if (
							ctrl instanceof
								Packages.mindustry.ai.types.FormationAI &&
							ctrl.leader
						) {
							tx = ctrl.leader.x;
							ty = ctrl.leader.y;
							...
							ctrl.targetPos &&
							ctrl.targetPos.x !== undefined
						) {
							tx = ctrl.targetPos.x;
							ty = ctrl.targetPos.y;
						} else if (
							u.targetFlag &&
							u.targetFlag.x !== undefined
						) {
							tx = u.targetFlag.x;
							ty = u.targetFlag.y;
					...
					let isMining = false;
					try {
						if (u.mining && u.mining()) isMining = true;
					} catch (e) {}
...
interceptor.add('track', (args) => {
	...
	if (arg1 === 'rts') {
		...
		notify(
			'[lightgray]RTS Tracking ' +
				(trackRtsEnabled ? '[green]ON' : '[scarlet]OFF')
		);
	...
	if (arg1 === 'rec') {
		...
		notify(
			'[lightgray]Rally Point Tracking ' +
				(trackRecEnabled ? '[green]ON' : '[scarlet]OFF')
		);
	...
	if (
		...
		Groups.player.each((p) => {
			if (Strings.stripColors(p.name).toLowerCase().includes(arg1))
				found = p;
		});
		if (found) {
			trackedPlayer = found;
		...
		trackEnabled = interceptor.parseToggle(
			trackEnabled,
			arg1 === 'toggle' ? args[2] : arg1
		);
		if (!trackEnabled) {
			trackedPlayer = null;
			unitCommanders = {};
		...
		notify(
			'[lightgray]Tracking Display ' +
				(trackEnabled ? '[green]ON' : '[scarlet]OFF')
		);
	}
});
			...
			if (u && u.isAdded()) {
				...
				if (clusters) {
					for (let k in clusters) {
						...
						let dist =
							Math.abs(avgX - avgTx) + Math.abs(avgY - avgTy);
